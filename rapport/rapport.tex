...
parser. À noter que le programme ne nécessite pas de système d'exploitation
...
généralement utilisé pour ce type de tache s'appelle un parser, qui va
...
syntaxes ou de problèmes de définitions de variables ou de fonctions. Il faudra
...
une condition. Un \textit{if} peut-être suivi d'un bloque \textit{else} si
...
\begin{itemize}
  \item \textbf{c++} : indique que l'on travaille avec du cpp et non du c
  \item \textbf{interactive} : utile quand on utlise \textbf{std::in}. Le
    ...
    (plus lent mais permet de lutter contre les ambiguités)
...
\begin{itemize}
  ...
  \item \textbf{float :} similaire aux entiers sauf qu'ici on a obligatoirement
    un point suivit d'une suite de chiffres à la fin.
...
lexeur pour écrire des règles de grammaires. Nous détaillerons la
...
\begin{itemize}
  ...
  \item Le caratère \textbf{'|'} est utilisé lorsque la règle est composée de
    ...
    les fonction, une où l'on spécifie le type de retour et l'autre ou on ne le
...
On peut voif sur ce diagramme que la classe possède des méthodes de créations
...
Pour implémenter la table des symbole, nous nous sommes inspiré du schéma
...
en remontant l'arborescence de la table jusqu'aux \textbf{scope} global. Les
...
syntaxe sont bloquantes, et cause un arrêt du parseur avec un retour non nul.
...
Si des nouvelles fonctionnalités seront apportées pour rendre le langage plus
puissant (et surtout pleinement utilisable), nous souhaitons tout de même
conserver une certaine simplicité. Le projet doit resté présentable à des gens
qui n'ont jamais codé et qui souhaitent simplement découvrir. Il n'y aura donc
pas d'ajout d'éléments complexe comme des pointeurs par exemple.\\
...
serra plus facile de fournir différentes implémentations du langage en gardant
...
avions choisit python pour la traduction. La création d'un compilateur pourrait
donc s'avérer très complexe (mais possible). La meilleur façon de créer un
...
projet. Il y a la possibilité de compéter le langage en lui ajoutant de
nouvelles fonctionnalités ou encore de complètement changer de sujet en
remplaçant le transpileur par un des autres outils que nous avons cité.
...
L'objectif du projet été de créer un langage transpilé simple pouvant être
présenté lors de portes ouvertes. Le langage devait permettre l'écriture de
fonctions, l'utilisation de variables, posséder des structures de contrôles if,
for et while et avoir un système de types. Pour cette partie, l'objectif a été
atteint. De plus, nous devions réaliser un transpileur pour traduire notre code
en python. Nous avons pu fournir une solution fonctionnelle permettant de faire
des tests qui ne satisfait cependant pas nos attentes, le projet n'est donc pas
complètement terminé. Pour finir le transpileur, il faudrait générer un nouvel
arbre syntaxique spécifique à python et faire la traduction à partir de cet
AST.\\
...
de nouveaux éléments de grammaire pour ajouter de nouvelle fonctionnalités comme
des tableaux ou des structures par exemple. Enfin, il est aussi envisageable de
remplacer le transpileur par un interpréteur ce qui nécessiterait
l'implémentation de plus d'éléments pour gérer la mémoire par exemple.
