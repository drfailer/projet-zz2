...
permettre de reconnaitre des règle de grammaires dans le code source.
...
orienté objet pour gérer toutes les instructions du programme.
...
Pour la traduction du code il y a aussi plusieurs possibilités. Dans le cas ou
on aurait des instructions de bas niveau, il faudrait tout traduire en python
ce qui peut être complexe. Avec le modèle objet, on peut avoir un transpileur qui
se charge de traduire les différents éléments ou avoir des méthodes de
traductions pour chacun des éléments du programme. Il sera peut-être aussi
nécessaire de modifier le programme récupéré avant la traduction. À noter que
l'objectif du projet est de créer un langage assez simple, il n'y aura donc pas
d'optimisation avant la traduction.\\
...
représentation sous forme d'arbre, et donc la méthode qui emploi les objets. On
appel cela un arbre syntaxique ou \gls{ast}. Les arbres syntaxiques sont nés
avec la théorie des langages et comme on peut le voir dans cet article
\cite{compilerTICH}, les \gls{ast} sont très utilisés par les compilateurs car
ce sont des structures plus simples à manipuler que du texte. Voici l'arbre
syntaxique de la fonction \textit{add3} :
...
\subsection{Les recherches} % NOTE: jsp si ça doit être là
