...
{
    name=token,
    description={unité lexicale, comme un mot, générée par un lexeur.}
}
...
Ce projet s'inscrit dans le cadre du projet commun de 2\up{ème} année de la
filière génie logiciel de l'école d'ingénieur ISIMA dont l'objectif est la
création d'outils de démonstration servant à présenter la filière. Notre travail
a consisté en la création d'un langage de programmation transpilé simple avec
une syntaxe similaire à celle du C. Le programme doit lire un fichier de code
source et le traduire en code python. Il doit être capable de détecter les
erreurs de syntaxe et de les spécifier à l'utilisateur. De plus, le langage est
à typage statique, de ce fait, le programme doit aussi être capable de détecter
...
This project is a part of the common second year project of the software
development pathway in the engineering school of ISIMA. The objective is to
create a tool for presenting software development. Our work consisted in the
creation of a simple transpiled programming language with a syntax similar to C
language. The program has to be able to read a source code file and generate a
python program. It must detect syntax errors in the and report theme to the
user. Furthermore, the language is statically typed so the program also has to
...
outils qui permettent la réalisation de notre langage. Enfin, nous présenterons
notre solution pour faire la traduction du code en python avec le transpileur
avant d'achever ce rapport par un bilan de projet.\\
...
L'objectif du projet est de créer un outil qui pourra servir à faire des
...
\cite{compilerFlexBison}. Ces deux outils vont nous permettre de gagner du temps
et d'avoir un code beaucoup plus efficace car les codes qu'ils génèrent sont
très performants.
...
assez simple, il n'y aura donc pas d'optimisation avant la traduction. Pour
simplifier et gagner du temps, nous choisirons l'option d'avoir des méthodes de
traduction des objets représentant éléments du langage.\\
...
manière formelle, nous utiliserons la \gls{bnf}* qui est un métalangage qui
permet de décrire des langages de programmations \cite{bnf}. Commençons par voir
un exemple avant de décrire les règles de notre langage.

\subsubsection*{Exemple}

Sur l'exemple ci-dessous, on définit quatre règles de grammaire. La première est
la règle est \textit{<bonjour>}, elle permet juste de reconnaître le mot
\textit{"bonjour"}. Les deux règles suivantes permettent de reconnaître les
chiffres et les lettres. La syntaxe \textbf{'0'-'9'} permet de dire que l'on
reconnaît les caractères compris entre 0 et 9 (et on a la même chose pour les
lettres). Enfin, la dernière règle décrit les suites de chiffres et de lettres. Le
symbole \textbf{|} est un \textbf{ou} entre les règles \textit{<chiffre>} et
\textit{<lettre>}, ce qui signifie que l'on a soit un chiffre, soit une lettre.
À la fin, on a le symbole \textbf{*} qui s'applique aux éléments entre
parenthèses et qui permet de dire que ces éléments sont présents zéro ou
\textit{n} fois. Si l'on souhaite avoir au moins un élément, on peut remplacer
\textbf{*} par \textbf{+}.
...
<chiffre> ::= '0'-'9'
<lettre> ::= 'a'-'z'
<suiteChiffresLettres> ::= ( <chiffre> | <lettre> )*
\end{grammar}\leavevmode\newline

\subsubsection*{Les variables}

Pour stocker des données, notre langage utilise des variables dont la convention
de nommage est la même qu'en \textbf{C} \cite{identifiers}. Le langage est à
typage statique, ce qui signifie que toutes les variables doivent être déclarées
avec un type avant d'être utilisées. Pour l'instant, nous possédons trois types
: les entiers (\textit{int}), les nombres à virgule flottante (\textit{flt}) et
les caractères (\textit{chr}). Voici la syntaxe pour déclarer une variable:
...
La syntaxe pour les fonctions est la même qu'en Rust\cite{rust}. Pour définir des
...
transpileur. La table \ref{ops} décrit les opérations possibles dans notre
langage.
...
le diagramme de la figure 2, la classe \lstinline{Statement} permet de
transmettre un \lstinline{Block} par héritage aux structures de contrôle et aux
fonctions.
...
binaire. Pour ne pas trop alourdir le schéma de la figure 3, seul la
classe \textit{AddOP} a été représentée, cependant, toutes les fonctions
décrites dans partie \ref{sec:operator} ont une classe qui hérite de
\lstinline{OperationBinaire}. \clearpage
...
\textbf{Type} est une énumération qui possède les éléments visibles sur la
figure 4.
...
voir sur le diagramme de la figure 5, cette classe possède deux attributs,
...
nous permettra de générer le code du lexeur avec l'outil Flex. Ce lexeur va nous
permettre de découper le texte en unités lexicales, les tokens. Nous verrons dans
...
\begin{itemize}
  \item[$\bullet$] \textbf{c++} : indique qu l'on travaille avec du cpp et non du c
  \item[$\bullet$] \textbf{interactive} : utile quand on utilise \textbf{std::in}. Le
  ...
  \item[$\bullet$] \textbf{noyywrap} : ne pas appeler \textbf{yywrap()} qui permet de
    parser plusieurs fichiers
  \item[$\bullet$] \textbf{nodefault} : pas de scanner par défaut (=> on doit tout
    implémenter)
  \item[$\bullet$] \textbf{outfile:"file.cpp"} : permet de définir le fichier de sortie
...
\begin{itemize}
  \item[$\bullet$] \textbf{alpha :} un caractère alphabétique est composé d'une lettre
    minuscule ou d'une lettre majuscule.
  \item[$\bullet$] \textbf{digit :} les chiffres sont les caractères entre 0 et 9.
  \item[$\bullet$] \textbf{int :} les entiers correspondent à une suite de chiffres et
    peuvent être positifs ou négatifs.
  \item[$\bullet$] \textbf{float :} similaire aux entiers sauf qu'ici on a obligatoirement
    un point suivi d'une suite de chiffres à la fin.
  \item[$\bullet$] \textbf{char :} les caractères sont toujours écrits entre \textbf{'}
    (par exemple \textbf{'a'}).
  \item[$\bullet$] \textbf{identifier :} correspond aux noms de fonctions et de variables
...
\begin{itemize}
  \item On donne le nom de la règle suivie de ':'.
...
Le choix de la réprésentation objet a été fait car cette représentation est plus
simple à comprendre et à visualiser. Cependant, dans notre cas, elle est plus
complèxe à générer : on peut générer du bytecode au fil de l'exécution du
parseur, en utilisant des \textcolor{orange}{goto} pour sauter de bloc
d'instructions en bloc d'instructions. Pour le code objet, les éléments à
l'intérieur des blocs doivent être créés avant le bloc, et le bloc est détecté
avant les instructions, il faut donc les stocker.


\clearpage{}

\subsection{Fabrique à programmes}\label{sec:fabprog}
...
le programme. Le diagramme de cette classe est visible sur la figure 6.
...
On peut voir sur ce diagramme que la classe possède des méthodes de création
...
présentée dans cet article \cite{power2000symbol} que nous avons simplifiée pour
la récupération et la recherche de symboles.\\

Commençons par étudier la composition d'un symbole. Comme on peut le voir sur la
figure 7, chaque symbole est composé d'un nom qui lui sert d'identifiant, d'un
type et d'un genre. Le genre est une énumération qui suit le même principe que
dans \cite{power2000symbol}. Le type du symbole est une liste de \textbf{Type}
qui est une énumération décrite dans l'AST, il a la même utilité que pour les
éléments typés (voir la section \ref{sec:eltTypes}). Ici nous stockons une liste
de \textit{Type} et non juste un \textit{Type}. En effet le type d'une variable
est bien composé d'un seul \textit{Type} (une liste de un élément) mais le type
des fonctions ne correspond pas uniquement au type de la valeur de retour. Le
type d'une fonction comprend aussi le type de chacun des paramètres.
...
de la figure 8 \cite{symtableGenius}. Ici la table des symboles est une table de
...
permettra l'accès aux valeurs avec l'identifiant. De plus, la table possède une
liste de sous tables qui est un pointeur sur la table mère qui sera utilisé par
la méthode \textbf{lookup} pour la recherche de symbole comme expliqué
précédemment. À noter que la méthode \textbf{lookup} ne renvoie pas directement
un symbole. Elle utilise la classe \textbf{\lstinline{std::optional}} et renvoie
une valeur vide dans le cas où le symbole recherché n'a pas été trouvé, ce qui
est pratique étant donné le fait que C++ ne possède pas de valeur nulle. Une
autre solution aurait été de retourner un pointeur.
...
dans un nouveau \textit{scope}. La structure de cette classe est visible sur la
figure 10.
...
Comme on peut le voir ci-dessous sur le diagramme de la figure 11, c'est une
classe très simple qui ne possède que deux attributs. Le premier est un flux de
caractères qui va permettre de stocker tous les messages d'erreurs. Le second
est un booleen, il est initialisé à \textit{false} au départ et passe à
\textit{true} quand une erreur est enregistrée. Cet attribut sert à savoir si on
doit compiler ou pas quand le parseur a consommé tout le texte. En effet, seules
les erreurs de syntaxe sont bloquantes, et causent un arrêt du parseur avec un
retour non nul. Cependant, il n'est pas nécessaire de planter au niveau du
parseur pour les erreurs de types. De ce fait il nous est possible de récupérer
toutes les erreurs de type dans un programme qui ne comporte pas d'erreurs de
syntaxe et donc de rendre la correction du programme plus rapide.
...
classe possède aussi deux méthodes permettant l'ajout de nouvelles erreurs et de
nouveaux avertissements. La différence entre les deux est que les avertissements
ne vont pas empêcher la transpilation. Par exemple, un avertissement peut être
levé lors de l'affectation d'un caractère à une variable de type entier. Ici,
l'avertissement va permettre de signifier à l'utilisateur que les types
diffèrent, cependant, ce dernier souhaite peut-être extraire le numéro ASCII du
caractère assigné, il ne faut donc pas stopper le programme dans ce genre de
cas.\\~\\
...
Une seconde fonction de vérification de type a été écrite, mais celle-ci génère
...
Pour plus de simplicité, la conversion des types au niveau des appels de
fonctions ne sera pas prise en charge. De ce fait, si une fonction prend en
entrée un entier, il faudra obligatoirement lui donner une variable ou une
valeur de type entier en paramètre pour ne pas avoir d'erreur. La conversion de
type au niveau des variables peut cependant se faire avec la fonction
\textit{set}, le manque de cette fonctionnalité n'est donc pas un problème.\\
...
de donner un sens au code source ainsi que de vérifier qu'il est correct. La
...
\subsection{Les recherches bibliographiques}
...
On peut donc voir sur ce premier diagramme (figure 12), que nous comptions
développer les éléments les uns après les autres. Nous comptions commencer par
l'AST puis implémenter le parseur, la table des symboles et le transpileur. À
noter que nous n'avions pas encore prévu de créer un objet pour la gestion des
erreurs. Ce diagramme est disponible en index \ref{appendix:expectedgantt} pour
plus de lisibilité.
...
Sur le diagramme réel (figure 13), on peut constater que nous n'avons pas suivi
le plan que nous avions défini pour les raisons expliquées plus haut. Cependant,
on peut tout même voir que la table des symboles et le transpileur sont toujours
fait séparément du reste. Si on observe les trois premières parties, on peut
voir qu'elles correspondent à l'implémentation de l'AST et du parseur. De plus,
si on compare la durée, on peut constater que la conception de ces deux premiers
...
rapide que prévu mais nous avons dû implémenter la gestion d'erreurs en plus. On
peut enfin constater qu'étant donné le fait que les deux premières parties ont
été plus longues, le temps consacré à l'implémentation du transpileur a été
...
Pour palier ce problème, nous pourrions ajouter une nouvelle règle de
...
\subsection{Récupération des éléments de code}
...
On peut voir sur les figures 14 et 15 l'affichage des erreurs sur un
programme test.
...
Le transpileur n'a au final pas été fait comme nous l'avions prévu au départ,
cependant nous avons pu implémenter une solution qui nous permet de tester la
génération de code python. Notre solution fonctionne car le langage est très
simple mais il y a tout de même des problèmes. Le premier gros problème est le
fait que les classes de l'AST de notre langage se chargent de la traduction. Il
aurait été préférable que les classes de l'arbre n'aient aucun lien avec le
transpileur. En effet, si nous souhaitions changer le langage cible pour la
traduction, ou encore remplacer le transpileur par un compilateur, il faudrait
modifier l'AST, ce qui ne devrait pas être le cas. De plus, comme nous l'avons
dit, si nous souhaitions ajouter de nouvelles fonctionnalités au langage, cela
pourrait entrainer beaucoup de problèmes.
...
Voici un exemple simple (figure 16) de la fonction de fibonacci codée avec notre
langage. Comme on peut le voir sur la figure ci-dessous, le programme demande un
...
Sur la figure 17, on peut voir le résultat lorsqu'on entre la valeur 8.
...
\clearpage
On peut voir sur la figure 18, le code python généré par le transpileur.
...
des gens qui n'ont jamais codé et qui souhaitent simplement découvrir la
programmation. Il n'y aura donc pas d'ajout d'éléments complexes comme des
